...
const PizzaDetails = ({ pizza, setModal }) => {
  ...
  const [addditionalToppingPrice, setAddditionalToppingPrice] = useState(0);
  const [price, setPrice] = useState(0);
  const { addToCart } = useContext(CartContext);

  useEffect(() => {
    size === 'small' ? setPrice(parseFloat(pizza.priceSm + addditionalToppingPrice).toFixed(2)) : 
    size === 'medium' ? setPrice(parseFloat(pizza.priceSm + addditionalToppingPrice).toFixed(2)) :
    size === 'large' ? setPrice(parseFloat(pizza.priceSm + addditionalToppingPrice).toFixed(2)) :
    null;
  }, [size, pizza.priceSm, addditionalToppingPrice]);
  ...
  useEffect(() => {
    if (additionalTopping.length > 0) {
      ...
      setAddditionalToppingPrice(toppingPrice);
    } else {
      setAddditionalToppingPrice(0);
    }
  }, [additionalTopping]);
